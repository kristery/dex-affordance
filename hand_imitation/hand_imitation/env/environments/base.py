# Original Source: https://github.com/ARISE-Initiative/robosuite/blob/master/robosuite/models/base.py
# Code is modified for simplicity and compatibility
...
from hand_imitation.env.utils.errors import SimulationError
from hand_imitation.env.utils.mujoco_py_renderer import MujocoPyRenderer
...
class EnvMeta(type):
    ...
    def __new__(meta, name, bases, class_dict):
        ...
        _unregistered_envs = [
            "MujocoEnv", "RobotEnv", "ManipulationEnv", "SingleArmEnv",
            "TwoArmEnv"
        ]
...
class MujocoEnv(metaclass=EnvMeta):
    ...
    def initialize_time(self, control_freq):
        ...
        """
        self.cur_time = 0
        self.model_timestep = self.mjpy_model.opt.timestep
    ...
    def _reset_internal(self):
        ...
        # By default, 0 for visual/collision, 1 for visual only and 2 for collision only
        if self.has_renderer and self.viewer is None:
            self.viewer = MujocoPyRenderer(self.sim)
            self.viewer.viewer.vopt.geomgroup[2] = (
            ...
            self.sim._render_context_offscreen.vopt.geomgroup[2] = (
                1 if self.render_collision_mesh else 0)
    ...
    def clear_objects(self, object_names):
        ...
        object_names = {object_names
                        } if type(object_names) is str else set(object_names)
    ...
    def dump(self) -> Dict[str, np.ndarray]:
        ...
        return {"qpos": qpos, "qvel": qvel, "qacc": qacc}

    def dump_mujoco_model(self) -> Dict[str, np.ndarray]:
        data = {}
        for key in ["body_pos", "body_quat", "site_pos", "site_quat"]:
            data[key] = getattr(self.mjpy_model, key)[:].copy()
        return data

    def pack(self, sim_data: Dict[str, np.ndarray]):
        self.data.qpos[:] = sim_data['qpos']
        self.data.qvel[:] = sim_data['qvel']
        self.data.qacc[:] = sim_data['qacc']

    def pack_mujoco_model(self, model_data: Dict[str, np.ndarray]):
        self.mjpy_model.body_pos[:] = model_data['body_pos']
        self.mjpy_model.body_quat[:] = model_data['body_quat']
        self.mjpy_model.site_pos[:] = model_data['site_pos']
        self.mjpy_model.site_quat[:] = model_data['site_quat']

    def get_joint_qpos_indices_from_names(self, names: List[str]) -> np.ndarray:
        addrs = []
        for name in names:
            addr = self.mjpy_model.get_joint_qpos_addr(name)
            if isinstance(addr, (tuple, list)):
                addrs += list(range(*addr))
            else:
                addrs.append(addr)
