# https://github.com/ARISE-Initiative/robosuite/blob/master/robosuite/models/arenas/table_arena.py
# Different from RoboSuite source, add functionality for YCB and primitive objects
import numpy as np
from hand_imitation.env.models.arenas.base import Arena
from hand_imitation.env.utils.mjcf_utils import (array_to_string, string_to_array,
                                                 xml_path_completion)
from hand_imitation.env.models.objects import YCBObject, PrimitiveObject, SHAPENETObject
...
class TableArena(Arena):
    ...
    def __init__(
        ...
        self.bottom_pos = np.array(bottom_pos)
        self.center_pos = np.array([0, 0, -self.table_half_size[2]]) + self.table_offset + self.bottom_pos
    ...
    def add_ycb_object(self, object_name, pos, quat, free=True, density=1000, idn=0, scale=1, **kwargs):
        ycb_object = YCBObject(object_name, free, density, idn, scale=scale, **kwargs)
        self.objects.append(ycb_object)
        assert isinstance(pos, (np.ndarray, list, tuple))
        pos[2] += self.table_top_abs[2]
        ycb_object.merge_into_xml(self, pos, quat)

    
    def add_shapenet_object(self, category, object_name, pos, quat, free=True, density=1000, idn=0, scale=1, **kwargs):
        shapenet_object = SHAPENETObject(category, object_name, free, density, idn, scale=scale, **kwargs)
        self.objects.append(shapenet_object)
        assert isinstance(pos, (np.ndarray, list, tuple))
        pos[2] += self.table_top_abs[2]
        shapenet_object.merge_into_xml(self, pos, quat)


    def add_primitive_object(self, primitive_name, primitive_size, pos, quat, free=True, density=1000, idn=0, **kwargs):
        primitive_object = PrimitiveObject(primitive_name, primitive_size, free, density, idn, **kwargs)
        self.objects.append(primitive_object)
        assert isinstance(pos, (np.ndarray, list, tuple))
        pos[2] += self.table_top_abs[2]
        primitive_object.merge_into_xml(self, pos, quat)
