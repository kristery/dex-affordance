...
from hand_imitation.env.utils.mjcf_utils import array_to_string, find_elements
...
def build_mug(object_name, density=1000, idn=0, scale=1, **kwargs):
    ...
    for i in range(12):
        size = array_to_string(np.array([0.028, 0.01, 0.08]) / 2 * scale)
        theta = np.deg2rad(i * 30)
        pos = array_to_string(np.array([0.042 * np.sin(theta) - 0.006, -0.042 * np.cos(theta), 0.0015]) * scale)
...
class SHAPENETObject:
    def __init__(self, category, object_name, free=True, density=1000, idn=0, scale=1, **kwargs):
        object_full_name = object_name
        category_full_name = 'shapenet_'+category
        visual_file = xml_path_completion(
            f"{category_full_name}/visual/{object_full_name}/model_transform_scaled.stl")
       
       
        collision_files = sorted(glob.glob(xml_path_completion(f"{category_full_name}/collision/{object_full_name}/*.stl")))
