...
class MuNet(nn.Module):
    def __init__(self, obs_dim, act_dim, hidden_sizes=(64,64),
        ...
        self.fc0   = nn.Linear(obs_dim - obj_size, hidden_sizes[0])
        self.fc1   = nn.Linear(hidden_sizes[0] + emb_size, hidden_sizes[1])
        self.fc2   = nn.Linear(hidden_sizes[1], act_dim)
        
        self.fc_emb = nn.Linear(obj_size, emb_size)
    ...
    def forward(self, x):
        ...
        original_obs = out[:, :-self.obj_size]
        obj_obs = out[:, -self.obj_size:]

        out1 = F.tanh(self.fc0(original_obs))
        out2 = F.softmax(self.fc_emb(obj_obs), dim=1)
